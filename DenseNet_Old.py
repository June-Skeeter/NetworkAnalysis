...
def TTV_Split(iteration,params,X,y):
    ...
    # print(indicies)
    print(y.shape)
    X_train,X_test,y_train,y_test,i_train,i_test,ones_train,ones_test=train_test_split(X,y,indicies,ones, test_size=.1, random_state=params['iteration'])#0.2s
    if params['Validate'] == True:
        X_train,X_val,y_train,y_val,i_train,i_val,ones_train,ones_val=train_test_split(X_train,y_train,i_train,ones_train, test_size=.11, random_state=params['iteration'])#0.25s
